...
import { monthlyPlans, yearlyPlans } from "../components/config/data"; // Import the data
      ...
      <div className="mx-auto mt-16 grid max-w-lg grid-cols-1 items-center gap-y-6 gap-x-10 sm:mt-20 sm:gap-y-0 lg:max-w-6xl lg:grid-cols-3">
        {plans.map((plan, index) => (
          <PricingCard
            ...
            buttonText={
              plan.plan === "Expert" ? "Coming Soon" : "Get started today"
            }
...
const PricingCard = ({
  ...
  const handleNavigation = () => {
    navigate("/payment/member", {
      state: { subscriptionDetails: { plan, fee: price } },
    });
  };
    ...
    >
      ...
      <h3
        className={`text-base font-semibold ${
          isPremium ? "text-indigo-400" : "text-indigo-600"
        }`}
      >
        {plan}
      </h3>
